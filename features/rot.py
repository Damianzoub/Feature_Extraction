...
def compute_rot(df, id_col, time_col, lat_col, lon_col):
    ...
    # Heading i
    df['vx_i'] = (df[lon_col] - df['lon_prev']) / ((df[time_col] - df['time_prev']).dt.total_seconds().replace(0, 1))
    df['vy_i'] = (df[lat_col] - df['lat_prev']) / ((df[time_col] - df['time_prev']).dt.total_seconds().replace(0, 1))
    heading_i = np.arctan2(df['vy_i'], df['vx_i'])

    # Heading i+1
    df['vx_ip1'] = (df['lon_next'] - df[lon_col]) / ((df['time_next'] - df[time_col]).dt.total_seconds().replace(0, 1))
    df['vy_ip1'] = (df['lat_next'] - df[lat_col]) / ((df['time_next'] - df[time_col]).dt.total_seconds().replace(0, 1))
    heading_ip1 = np.arctan2(df['vy_ip1'], df['vx_ip1'])

    # Rate of Turn (ROT)
    df['heading_diff'] = heading_ip1 - heading_i
    df['rot_time_diff'] = (df['time_next'] - df[time_col]).dt.total_seconds().replace(0, 1)
